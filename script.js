/* ═══════════════════════════════════════════════════════════════
   POCKET SPROUT — MAIN SCRIPT (FIXED VERSION)
   ═══════════════════════════════════════════════════════════════ */
...
const audio = {
    ...
    // FIX #5: Improved rain sound that properly restarts
    startRainSound(){
        if(!this.ctx)this.init();
        if(this.ctx.state==='suspended')this.ctx.resume();
        // Clean up any existing rain sound first
        this.stopRainSound();
    ...
    stopRainSound(){
        ...
        if(this.rainGain&&this.rainOsc){
            try{
                ...
                setTimeout(()=>{try{this.rainOsc.stop()}catch(e){}},600);
            }catch(e){}
        }
...
function init(){
    ...
    // FIX #5: Resume rain sound if it was on when app closed
    if(state.isRainOn){
        applyTheme();
    }
...
// FIX #2: Only update the sky moon element, not the status area moonIcon (removed duplicate)
function updateMoonPhase(){
    const p=getMoonPhase();
    const me=document.getElementById('moonElement');
    if(me)me.style.setProperty('--moon-phase',p.phase+'%');
    // Update the season indicator to include day info without moon icon
    updateSeason();
}
function isNewMoon(){return getMoonPhase().name==='New Moon'}
// FIX #2: Season indicator shows day and season, no moon icon here
function updateSeason(){
    const s=SEASONS[state.season%4];
    const i=document.getElementById('seasonIndicator');
    if(i)i.textContent=`Day ${state.day} · ${s.icon} ${s.name}`;
}
...
// FIX #1: Completely rewritten plant rendering with proper branch stems for multiple flowers
function renderPlant(containerId,dna,stage,scarsOverride=null){
    ...
    // Flowers with proper branching stems at stage 3+
    if(stage>=3){
...
function render(){
    ...
    // FIX #2: Day display removed from here - now in seasonIndicator
    ...
    // Update season indicator with current day
    updateSeason();
}
...
function applyTheme(){
    ...
    // FIX #5: Always sync rain sound with state
...
function attemptSpawnFirefly(){
    ...
    // FIX #6: Only spawn guardian if not already active
...
// FIX #6: Completely rewritten firefly spawn with proper animation classes
function spawnVisualFirefly(fam,isGuardian){
    ...
    // Non-guardians fade out after 12 seconds
    if(!isGuardian){
        setTimeout(()=>{
            if(ff.parentNode){
                ff.style.transition='opacity 1s ease-out, transform 1s ease-out';
                ff.style.opacity='0';
                ff.style.transform='scale(0.3)';
...
// FIX #6: Rewritten guardian activation - no repositioning, just activation glow and timed fadeout
function activateGuardian(fam,el){
    ...
    // Guardian lasts 60 seconds then fades out
    setTimeout(()=>{
        // Fade out animation
...
function confirmHarvest(){
    ...
    state.generation++;state.day=1;state.stage=1;state.growth=0;
    state.water=50;state.sun=50;state.love=50;
    state.scars=[];state.timeAtZero=0;state.name='Sprout';
    ...
    setupWorld();renderPlant('plantGroup',state.dna,state.stage);
    updateSeason();updateUI();saveState();
}
...
// FIX #3: Add reset confirmation overlay handler
...
// FIX #5: Improved visibility handler that properly restarts rain sound
function handleVisibility(){
    if(document.visibilityState==='visible'){
        processOfflineProgress();
        if(audio.ctx)audio.ctx.resume();
        // Restart rain sound if it was on
        if(state.isRainOn){
            audio.startRainSound();
        }
        document.body.classList.remove('paused');
        // Respawn active guardians
        state.activeGuardians.forEach(i=>{
            if(!document.querySelector(`.guardian[data-family="${i}"]`)){
                spawnVisualFirefly(i,true);
            }
        });
    }else{
        saveState();
        // Stop rain sound when hidden
...
// FIX #3: Full reset function for player use (not debug)
function fullReset(){
    resetGame(false);
...
// FIX #4: Debug tap threshold increased to 20
