...
const FIREFLY_FAMILIES = [{name:'Ember',hue:15,power:'warmth',desc:'Restores sun vitality',effect:'sun'},{name:'Citrine',hue:45,power:'growth',desc:'Accelerates growth',effect:'growth'},{name:'Verdant',hue:120,power:'vitality',desc:'Boosts overall health',effect:'health'},{name:'Aqua',hue:180,power:'hydration',desc:'Restores water',effect:'water'},{name:'Azure',hue:210,power:'calm',desc:'Slows decay',effect:'slow'},{name:'Violet',hue:270,power:'dreams',desc:'Increases luck',effect:'luck'},{name:'Rose',hue:330,power:'love',desc:'Fills with affection',effect:'love'},{name:'Pearl',hue:0,sat:15,power:'mystery',desc:'Random powerful effects',effect:'random'}];
...
const STAGES = ['Seed','Seedling','Sprout','Bloom','Flourish'];
const STAGE_THRESHOLDS = [0,400,1500,3500,7000];
...
const CONFIG = {decayRate:{water:0.035,sun:0.035,love:0.02},growthRate:0.35,tickRate:1000,threshold:40,deathTimeLimit:12*60*60*1000,offlineDecayFactor:0.08,singCooldown:300000,fertilizeCooldown:180000,maxFireflyPerFamily:50};
const DREAMS = ["dreamed of distant mountains","listened to rain and remembered forests","felt roots intertwining with the past","saw gardens yet to bloom","remembered its first sunrise","heard whispers from sleeping seeds","imagined dancing with fireflies","felt the heartbeat of the earth","dreamed of butterfly wings","whispered secrets to the moon","envisioned endless meadows","imagined being the tallest tree","heard the hum of bees","dreamed of catching raindrops","swayed with plants from other worlds","felt roots connecting everywhere","was wrapped in warm sunbeams","saw a garden of descendants","watched seasons change beautifully","remembered peaceful seed days","saw clouds forming shapes","tasted the first dewdrop","watched aurora lights dance","befriended a hummingbird","imagined flowers of every color","connected to ancient trees","taught seedlings to grow","heard distant waterfalls","dreamed of starlight leaves","lived where every day was spring","received wisdom from great oaks","bloomed under a double rainbow","sank roots into ancient soil","was painted by an artist","talked with the wind","grew glowing petals","lived in harmony with all","attracted phoenixes","had roots like rivers","was the first plant on a new planet","bonded with forest creatures","healed weary travelers","had musical leaves","remembered every kind word","felt warm snowflakes","grew wish-granting fruit","became home to ecosystems","relived your first notice","bloomed at perfect sunrise","felt the love of all gardeners"];
...
const audio = {
    ...
    toggleRainSound(on){if(!this.ctx)this.init();if(on){if(this.isRainPlaying)return;this.isRainPlaying=true;if(this.ctx.state==='suspended')this.ctx.resume();const buf=this.ctx.createBuffer(1,this.ctx.sampleRate*2,this.ctx.sampleRate),d=buf.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*0.1;this.rainOsc=this.ctx.createBufferSource();this.rainOsc.buffer=buf;this.rainOsc.loop=true;this.rainGain=this.ctx.createGain();this.rainGain.gain.value=0;this.rainOsc.connect(this.rainGain);this.rainGain.connect(this.ctx.destination);this.rainOsc.start();this.rainGain.gain.linearRampToValueAtTime(0.05,this.ctx.currentTime+2)}else{if(!this.isRainPlaying)return;this.isRainPlaying=false;if(this.rainGain){this.rainGain.gain.linearRampToValueAtTime(0,this.ctx.currentTime+1);setTimeout(()=>{if(this.rainOsc)this.rainOsc.stop()},1000)}}},
    water(){this.play(800,'sine',0.2)},sun(){this.play(300,'triangle',0.5)},love(){this.play(150,'sine',0.5)},err(){this.play(100,'sawtooth',0.2)},chime(){this.play(600,'sine',0.1);this.play(800,'sine',0.2)},
...
function updateTimeOfDay(){const t=getTimeOfDay();document.body.className=document.body.className.replace(/time-\w+/g,'');document.body.classList.add('time-'+t)}
...
function updateMoonPhase(){const p=getMoonPhase();const mi=document.getElementById('moonIcon'),me=document.getElementById('moonElement');if(mi)mi.textContent=p.icon;if(me)me.style.setProperty('--moon-phase',p.phase+'%')}
function isNewMoon(){return getMoonPhase().name==='New Moon'}
function updateSeason(){const s=SEASONS[state.season%4];const i=document.getElementById('seasonIndicator');if(i)i.textContent=s.icon+' '+s.name}
...
function renderPlant(containerId,dna,stage){
    const g=document.getElementById(containerId);if(!g||!dna)return;g.innerHTML='';
    const hasWilt=state.scars.includes('wilt'),hasBend=state.scars.includes('bend'),hasPale=state.scars.includes('pale');
    let cH=dna.colorH,cS=dna.colorS,cL=dna.colorL;if(hasPale){cS=Math.max(20,cS-30);cL=Math.min(70,cL+15)}
    ...
    const stemH=dna.stemHeight*(0.3+stage*0.175),stemTop=180-stemH,curve=(dna.stemCurve||0)+bendOff;
    if(stage>=1){const stem=createSVGElement('path');stem.setAttribute('d',`M100 180Q${100+curve} ${stemTop+stemH/2} ${100+curve/2} ${stemTop}`);stem.setAttribute('stroke',stemColor);stem.setAttribute('stroke-width',3+stage*0.5);stem.setAttribute('fill','none');stem.setAttribute('stroke-linecap','round');g.appendChild(stem)}
    if(stage>=1){const lc=Math.min(dna.leafCount||3,1+stage);for(let i=0;i<lc;i++){const t=0.25+(i/lc)*0.5,y=180-stemH*t,x=100+curve*t,side=i%2===0?-1:1,ang=(dna.leafAngle||45)*side+(hasWilt?wiltAng*side:0);renderLeaf(g,x,y,ang,dna.leafSize||1,leafColor,dna.leafShape||'round')}}
    if(stage===2){const bud=createSVGElement('ellipse');bud.setAttribute('cx',100+curve/2);bud.setAttribute('cy',stemTop);bud.setAttribute('rx',6);bud.setAttribute('ry',8);bud.setAttribute('fill',`hsl(${dna.flowerH},${dna.flowerS*0.5}%,${dna.flowerL-10}%)`);g.appendChild(bud)}
    if(stage>=3){renderFlower(g,100+curve/2,stemTop-5,dna.petalCount||5,dna.petalShape||'round',flowerColor,stage>=4?1.3:1);if(dna.flowerCount>=2&&stage>=4)renderFlower(g,100+curve*0.6+lean*-20,stemTop+20,dna.petalCount||5,dna.petalShape||'round',flowerColor,0.8);if(dna.flowerCount>=3&&stage>=4)renderFlower(g,100+curve*0.7+lean*15,stemTop+30,dna.petalCount||5,dna.petalShape||'round',flowerColor,0.7)}
}

function renderLeaf(g,x,y,angle,scale,color,shape){
    const leaf=createSVGElement('path'),sz=scale*15;
    const paths={pointed:`M0,0Q${-sz*0.5},${-sz} 0,${-sz*1.5}Q${sz*0.5},${-sz} 0,0`,heart:`M0,0C${-sz*0.8},${-sz*0.3} ${-sz*0.8},${-sz} 0,${-sz*1.2}C${sz*0.8},${-sz} ${sz*0.8},${-sz*0.3} 0,0`,oak:`M0,0Q${-sz*0.3},${-sz*0.4} ${-sz*0.5},${-sz*0.5}Q${-sz*0.3},${-sz*0.8} 0,${-sz*1.2}Q${sz*0.3},${-sz*0.8} ${sz*0.5},${-sz*0.5}Q${sz*0.3},${-sz*0.4} 0,0`,round:`M0,0Q${-sz*0.7},${-sz*0.7} 0,${-sz*1.3}Q${sz*0.7},${-sz*0.7} 0,0`};
    leaf.setAttribute('d',paths[shape]||paths.round);leaf.setAttribute('fill',color);leaf.setAttribute('transform',`translate(${x},${y})rotate(${angle})`);g.appendChild(leaf);
}

function renderFlower(g,x,y,count,shape,color,scale){
    const fg=createSVGElement('g');fg.setAttribute('transform',`translate(${x},${y})`);const sz=12*scale;
    const paths={pointed:`M0,0Q${sz*0.3},${-sz*0.7} 0,${-sz}Q${-sz*0.3},${-sz*0.7} 0,0`,wavy:`M0,0C${sz*0.4},${-sz*0.3} ${sz*0.2},${-sz*0.7} 0,${-sz}C${-sz*0.2},${-sz*0.7} ${-sz*0.4},${-sz*0.3} 0,0`,round:`M0,0Q${sz*0.5},${-sz*0.5} 0,${-sz}Q${-sz*0.5},${-sz*0.5} 0,0`};
    for(let i=0;i<count;i++){const p=createSVGElement('path');p.setAttribute('d',paths[shape]||paths.round);p.setAttribute('fill',color);p.setAttribute('transform',`rotate(${(360/count)*i})`);p.style.opacity=0.9;fg.appendChild(p)}
    const c=createSVGElement('circle');c.setAttribute('r',4*scale);c.setAttribute('fill','#fbbf24');fg.appendChild(c);g.appendChild(fg);
}

function renderPotPreview(){
    const g=document.getElementById('potPreviewGroup');
    if(g){g.innerHTML='';const body=createSVGElement('path');body.setAttribute('d','M72 30L128 30L118 70L82 70Z');body.setAttribute('fill',state.potColor);g.appendChild(body);const rim=createSVGElement('path');rim.setAttribute('d','M68 25L132 25L130 35L70 35Z');rim.setAttribute('fill',state.potColor);rim.style.filter='brightness(1.1)';g.appendChild(rim);if(state.potPattern&&state.potPattern!=='patNone'){const pat=createSVGElement('path');pat.setAttribute('d','M72 35L128 35L118 70L82 70Z');pat.setAttribute('fill',`url(#${state.potPattern})`);pat.style.opacity=0.9;g.appendChild(pat)}}
    updateMainPot();
}

function updateMainPot(){document.documentElement.style.setProperty('--pot-color',state.potColor);document.documentElement.style.setProperty('--pot-pat-color',state.potPatternColor);const p=document.getElementById('potPatternPath');if(p)p.setAttribute('fill',state.potPattern?`url(#${state.potPattern})`:'none')}

let lastTickTime=Date.now(),saveDebounceTimer=null;
function startGameLoop(){setInterval(gameTick,CONFIG.tickRate)}
function gameTick(){if(state.isDead||document.visibilityState!=='visible')return;lastTickTime=Date.now();decay();checkDeath();grow();processBuffs();processGuardians();attemptSpawnFirefly();if(!saveDebounceTimer){saveDebounceTimer=setTimeout(()=>{saveState();saveDebounceTimer=null},5000)}render()}

function decay(){
    const res=state.dna?.resilience||1,slow=state.buffs.find(b=>b.type==='slow')?0.5:1;
    if(state.isRainOn){state.water=Math.min(100,state.water+0.2);if(state.water>=95&&Math.random()>0.95)spawnFloatingText("Too Wet!","#ff6b6b")}else{state.water=Math.max(0,state.water-(CONFIG.decayRate.water/res)*slow)}
    if(state.isSunLampOn){if(isDaytime())state.sun=Math.min(100,state.sun+0.5);else if(!isNewMoon())state.sun=Math.min(100,state.sun+0.2);if(state.sun>90){state.water=Math.max(0,state.water-0.2);if(Math.random()>0.97)spawnFloatingText("Scorching!","orange")}}else{state.sun=Math.max(0,state.sun-(CONFIG.decayRate.sun/res)*slow)}
...
function checkDeath(){
    ...
    if(crisis){state.timeAtZero+=CONFIG.tickRate;const hrs=state.timeAtZero/(1000*60*60);
    ...
    }else{if(state.timeAtZero>0)state.timeAtZero=Math.max(0,state.timeAtZero-CONFIG.tickRate*0.5);if(state.scars.includes('dormant')&&getAverageVitality()>50)els.plantHero.classList.remove('dormant-plant')}
}
...
function grow(){
    ...
    let next=1;for(let i=1;i<STAGE_THRESHOLDS.length;i++)if(state.growth>=STAGE_THRESHOLDS[i])next=i+1;
    if(next>state.stage){state.stage=next;els.plantHero.classList.add('evolving');setTimeout(()=>els.plantHero.classList.remove('evolving'),2000);renderPlant('plantGroup',state.dna,state.stage);spawnFloatingText("✨ EVOLVED!","white");audio.chime()}
}

function processBuffs(){state.buffs=state.buffs.filter(b=>{b.remaining--;if(b.type==='water')state.water=Math.min(100,state.water+b.strength);if(b.type==='sun')state.sun=Math.min(100,state.sun+b.strength);if(b.type==='love')state.love=Math.min(100,state.love+b.strength);if(b.type==='growth')state.growth+=b.strength;if(b.type==='health'){state.water=Math.min(100,state.water+b.strength*0.3);state.sun=Math.min(100,state.sun+b.strength*0.3);state.love=Math.min(100,state.love+b.strength*0.3)}return b.remaining>0})}
function processGuardians(){state.activeGuardians.forEach(i=>{const f=FIREFLY_FAMILIES[i];if(f.effect==='water')state.water=Math.min(100,state.water+0.02);if(f.effect==='sun')state.sun=Math.min(100,state.sun+0.02);if(f.effect==='love')state.love=Math.min(100,state.love+0.02);if(f.effect==='growth')state.growth+=0.1;if(f.effect==='health'){state.water=Math.min(100,state.water+0.01);state.sun=Math.min(100,state.sun+0.01);state.love=Math.min(100,state.love+0.01)}})}
...
function render(){
    ...
    if(els.dayDisplay)els.dayDisplay.textContent=`Day ${state.day}`;
    ...
    const mood=getMood();if(els.plantMoodDisplay){els.plantMoodDisplay.textContent=mood.text;els.plantMoodDisplay.style.color=mood.color}
    ...
    if(els.evolutionBar){if(state.stage<5){const prev=STAGE_THRESHOLDS[state.stage-1]||0,next=STAGE_THRESHOLDS[state.stage];els.evolutionBar.style.width=Math.min(100,((state.growth-prev)/(next-prev))*100)+'%'}else{els.evolutionBar.style.width='100%'}}
    const scale=Math.min(1.4,1+(state.growth/6000));if(els.plantGraphics)els.plantGraphics.style.transform=`scale(${scale})`;
...
function updateMenuStats(){
    ...
    const ps=getPlantState();if(els.menuHealth){els.menuHealth.textContent=ps.label;els.menuHealth.style.color=ps.color}
    if(els.menuProgressBar){if(state.stage<5){const prev=STAGE_THRESHOLDS[state.stage-1]||0,next=STAGE_THRESHOLDS[state.stage];els.menuProgressBar.style.width=Math.min(100,((state.growth-prev)/(next-prev))*100)+'%'}else{els.menuProgressBar.style.width='100%'}}
    if(els.menuScars&&els.menuScarList){if(state.scars.length>0){els.menuScars.style.display='flex';els.menuScarList.textContent=state.scars.map(s=>({wilt:'Wilted',bend:'Bent',pale:'Faded',dormant:'Dormant'}[s]||s)).join(', ')}else{els.menuScars.style.display='none'}}
    if(els.menuInherited&&els.menuInheritedList){if(state.inheritedTraits.length>0){els.menuInherited.style.display='flex';els.menuInheritedList.textContent=state.inheritedTraits.map(id=>{const t=INHERITABLE_TRAITS.find(x=>x.id===id);return t?t.name:id}).join(', ')}else{els.menuInherited.style.display='none'}}
    if(els.btnHarvest)els.btnHarvest.classList.toggle('visible',state.stage>=5);
    if(els.greenhouseList){els.greenhouseList.innerHTML='';[...state.history].reverse().forEach((h,i)=>{const item=document.createElement('div');item.className='greenhouse-item';const col=h.dna?.flowerColor||`hsl(${h.dna?.flowerH||0},50%,50%)`;item.innerHTML=`<span><span class="gh-dot" style="background:${col}"></span>${h.name}</span><span style="opacity:0.5">Cycle ${h.gen}</span>`;item.onclick=()=>viewArchive(state.history.length-1-i);els.greenhouseList.appendChild(item)})}
}

function interact(type,e){
    if(state.isDead)return;if(e)e.stopPropagation();
    if(type==='rain'){state.isRainOn=!state.isRainOn;spawnFloatingText(state.isRainOn?"☁️ Rain ON":"☁️ Rain OFF","var(--accent-water)");applyTheme()}
    else if(type==='sun'){if(isDaytime()){state.isSunLampOn=!state.isSunLampOn;spawnFloatingText(state.isSunLampOn?"☀️ ON":"🌑 OFF","var(--accent-sun)")}else{if(isNewMoon()){spawnFloatingText("New Moon - No light","#888");return}state.isSunLampOn=!state.isSunLampOn;spawnFloatingText(state.isSunLampOn?"🌙 Moonlight ON":"🌙 OFF","#fef9c3");const beam=document.getElementById('moonlightBeam');if(beam)beam.classList.toggle('active',state.isSunLampOn)}audio.sun();applyTheme()}
    else if(type==='love'){if(state.love>=100){spawnFloatingText("Fully Loved!","var(--accent-love)");return}state.love=Math.min(100,state.love+15);spawnFloatingText("❤️","var(--accent-love)");audio.love();els.plantHero.classList.remove('plant-bloop');void els.plantHero.offsetWidth;els.plantHero.classList.add('plant-bloop');setTimeout(()=>els.plantHero.classList.remove('plant-bloop'),500)}
    render();
}

function applyTheme(){document.body.classList.toggle('sun-mode',state.isSunLampOn&&isDaytime());const rc=document.getElementById('rainContainer');if(rc)rc.classList.toggle('active',state.isRainOn);const rb=document.getElementById('rainbowContainer');if(rb)rb.classList.toggle('visible',state.isSunLampOn&&state.isRainOn);audio.toggleRainSound(state.isRainOn)}
...
function attemptSpawnFirefly(){if(els.menuOverlay.classList.contains('open'))return;if(Math.random()<(state.dna?.fireflyChance||0.05)){spawnVisualFirefly(Math.floor(Math.random()*8),false)}FIREFLY_FAMILIES.forEach((_,i)=>{if(hasGuardian(i)&&!activeBigFireflies.includes(i)&&Math.random()<0.008)spawnVisualFirefly(i,true)})}

function spawnVisualFirefly(fam,isG){
    const col=getFireflyColor(fam),ff=document.createElement('div');ff.className='firefly'+(isG?' guardian':'');ff.style.background=col;ff.style.color=col;ff.style.boxShadow=`0 0 ${isG?20:8}px ${col}`;ff.style.left=(10+Math.random()*80)+'%';ff.style.top=(15+Math.random()*50)+'%';
    ff.onclick=e=>{e.stopPropagation();if(isG)activateGuardian(fam,ff);else collectFirefly(fam,ff)};
    document.body.appendChild(ff);if(!isG)setTimeout(()=>{if(ff.parentNode)ff.remove()},12000);
}

function collectFirefly(fam,el){el.remove();audio.chime();if(!state.fireflies[fam])state.fireflies[fam]=0;if(state.fireflies[fam]<CONFIG.maxFireflyPerFamily){state.fireflies[fam]++;state.totalFireflies++;spawnFloatingText(`+${FIREFLY_FAMILIES[fam].name}!`,getFireflyColor(fam));if(state.fireflies[fam]===GUARDIAN_THRESHOLD)spawnFloatingText(`🏆 ${FIREFLY_FAMILIES[fam].name} Guardian!`,getFireflyColor(fam))}else{spawnFloatingText("Max collected!","#fff")}state.growth+=2}
function hasGuardian(i){return(state.fireflies[i]||0)>=GUARDIAN_THRESHOLD}

function activateGuardian(fam,el){if(activeBigFireflies.includes(fam))return;activeBigFireflies.push(fam);if(!state.activeGuardians.includes(fam))state.activeGuardians.push(fam);spawnFloatingText(`${FIREFLY_FAMILIES[fam].name} Guardian Active!`,getFireflyColor(fam));el.classList.add('guardian-active');el.style.pointerEvents='none';setTimeout(()=>{el.classList.add('guardian-fading');setTimeout(()=>{el.remove();activeBigFireflies=activeBigFireflies.filter(x=>x!==fam);state.activeGuardians=state.activeGuardians.filter(x=>x!==fam)},2000)},58000)}
...
function renderPotDesigner(){
    const cg=document.getElementById('potColorGrid');if(cg){cg.innerHTML='';POT_COLORS.forEach(c=>{const d=document.createElement('div');d.className='color-dot'+(state.potColor===c?' selected':'');d.style.background=c;d.onclick=()=>{state.potColor=c;renderPotDesigner();renderPotPreview()};cg.appendChild(d)})}
    const pg=document.getElementById('potPatternGrid');if(pg){pg.innerHTML='';PATTERNS.forEach(p=>{const unlocked=state.totalFireflies>=p.unlockAt;const btn=document.createElement('button');btn.className='pattern-btn'+(state.potPattern===p.id?' selected':'')+(!unlocked?' locked':'');btn.textContent=unlocked?p.name:`🔒 ${p.unlockAt}`;btn.disabled=!unlocked;btn.onclick=()=>{if(unlocked){state.potPattern=p.id;renderPotDesigner();renderPotPreview()}};pg.appendChild(btn)})}
    const pcg=document.getElementById('patternColorGrid');if(pcg){pcg.innerHTML='';PATTERN_COLORS.forEach(c=>{const d=document.createElement('div');d.className='color-dot'+(state.potPatternColor===c?' selected':'');d.style.background=c;d.onclick=()=>{state.potPatternColor=c;renderPotDesigner();renderPotPreview()};pcg.appendChild(d)})}
}
...
function confirmHarvest(){
    ...
    const tid=document.getElementById('inheritedTraitDisplay').dataset.traitId;if(!state.inheritedTraits.includes(tid))state.inheritedTraits.push(tid);
    const oldName=state.name;state.generation++;state.day=1;state.stage=1;state.growth=0;state.water=50;state.sun=50;state.love=50;state.scars=[];state.timeAtZero=0;state.name='Sprout';state.season=(state.season+1)%4;state.dna=generateDNA(state.dna);
    els.harvestOverlay.classList.remove('open');spawnFloatingText(`A seed from ${oldName} takes root...`,'var(--accent-growth)');setupWorld();renderPlant('plantGroup',state.dna,state.stage);updateSeason();updateUI();saveState();
}

function viewArchive(i){if(i<0||i>=state.history.length)return;const a=state.history[i];els.menuOverlay.classList.remove('open');els.archiveOverlay.classList.add('open');document.getElementById('archiveTitle').textContent=a.name;renderPlant('archivePlantGroup',a.dna,a.stage);const stats=document.getElementById('archiveStats');stats.innerHTML=`<div class="archive-stat"><span>Generation</span><span>${a.gen}</span></div><div class="archive-stat"><span>Days Lived</span><span>${a.days}</span></div><div class="archive-stat"><span>Stage</span><span>${STAGES[a.stage-1]||'Unknown'}</span></div>${a.scars?.length?`<div class="archive-stat"><span>Scars</span><span>${a.scars.join(', ')}</span></div>`:''}`}
function closeArchive(){els.archiveOverlay.classList.remove('open');els.menuOverlay.classList.add('open')}
function closeResetOverlay(){els.resetOverlay.classList.remove('open')}

function singToPlant(){if(Date.now()<state.singCooldownUntil)return;state.singCooldownUntil=Date.now()+CONFIG.singCooldown;checkSingCooldown();toggleMenu();spawnFloatingText("🎵 Singing...","#748ffc");let needed=STAGE_THRESHOLDS[state.stage]||1000;if(state.stage<4)needed=STAGE_THRESHOLDS[state.stage+1]-STAGE_THRESHOLDS[state.stage];const boost=needed*0.1;state.growth+=boost;setTimeout(()=>spawnFloatingText(`+${Math.floor(boost)} Growth!`,"#4ade80"),500);els.plantHero.classList.add('sing-glow');setTimeout(()=>els.plantHero.classList.remove('sing-glow'),3000);let i=0;const notes=[261,329,392,523];const iv=setInterval(()=>{if(i>=notes.length){clearInterval(iv);return}audio.play(notes[i],'triangle',0.3);i++},300)}
...
function processOfflineProgress(){
    const now=Date.now(),diff=(now-state.lastSave)/1000;if(diff<60)return;const hrs=diff/3600;
    ...
    let nested=null;if(hrs>=2&&Math.random()<0.4){const fi=Math.floor(Math.random()*8);if(!state.fireflies[fi])state.fireflies[fi]=0;state.fireflies[fi]++;state.totalFireflies++;nested=FIREFLY_FAMILIES[fi]}
    let dream=null;if(hrs>=1){const di=Math.floor(Math.random()*DREAMS.length);if(di!==state.lastDream){dream=DREAMS[di];state.lastDream=di}}
    if(diff>600){let tc='';if(dream)tc+=`<div style="font-style:italic;margin-bottom:10px;opacity:0.9;">While away, your plant ${dream}</div>`;tc+=`<div style="font-size:0.8rem;opacity:0.7;">You were away for ${hrs>=1?`${Math.floor(hrs)}h ${Math.floor((diff%3600)/60)}m`:`${Math.floor(diff/60)}m`}</div>`;const ps=getPlantState();tc+=`<div style="margin:8px 0;">Your plant is <span style="color:${ps.color};font-weight:bold;">${ps.label}</span></div>`;if(nested)tc+=`<div style="color:${getFireflyColor(FIREFLY_FAMILIES.indexOf(nested))};">🦋 A ${nested.name} firefly nested nearby!</div>`;if(wasHappy)tc+=`<div style="opacity:0.7;font-size:0.85rem;margin-top:5px;">Growth continued while you were away 🌱</div>`;document.getElementById('toastBody').innerHTML=tc;els.welcomeToast.classList.add('visible')}
    state.lastSave=now;saveState();
}
function closeToast(){els.welcomeToast.classList.remove('visible')}

function handleVisibility(){if(document.visibilityState==='visible'){processOfflineProgress();if(audio.ctx)audio.ctx.resume();if(state.isRainOn&&audio.rainGain)audio.rainGain.gain.linearRampToValueAtTime(0.05,audio.ctx.currentTime+0.5);document.body.classList.remove('paused');state.activeGuardians.forEach(i=>{if(!document.querySelector(`.guardian[data-family="${i}"]`))spawnVisualFirefly(i,true)})}else{saveState();if(audio.rainGain&&audio.ctx)audio.rainGain.gain.linearRampToValueAtTime(0,audio.ctx.currentTime+0.3);document.body.classList.add('paused')}}
...
function resetGame(preserveHistory=true){
    ...
    els.deathOverlay.classList.remove('open');els.plantHero.classList.remove('dead-plant','dormant-plant');setupWorld();renderPlant('plantGroup',state.dna,state.stage);renderPotPreview();updateUI();saveState();
}
...
function handleBackButton(e){const overlays=[{el:els.fireflyOverlay,close:closeFireflyLog},{el:els.potOverlay,close:closePotDesigner},{el:els.harvestOverlay,close:closeHarvestModal},{el:els.archiveOverlay,close:closeArchive},{el:els.helpOverlay,close:closeHelp},{el:els.resetOverlay,close:closeResetOverlay},{el:els.menuOverlay,close:toggleMenu}];let closed=false;for(const o of overlays){if(o.el&&o.el.classList.contains('open')){o.close();closed=true;break}}if(closed)history.pushState({popup:'none'},'')}

function spawnFloatingText(text,color){const el=document.createElement('div');el.className='floating-text';el.textContent=text;el.style.color=color;el.style.left='50%';el.style.top='40%';el.style.transform='translateX(-50%)';document.body.appendChild(el);setTimeout(()=>el.remove(),2000)}

let debugTapCount=0,debugTapTimer=null;
function handleDebugTap(){debugTapCount++;clearTimeout(debugTapTimer);debugTapTimer=setTimeout(()=>debugTapCount=0,2000);if(debugTapCount>=10)toggleDebug()}
...
function debugCycleMoon(){const m=document.getElementById('moonElement');if(m){const ps=[0,25,50,75,100,-75,-50,-25],c=ps.indexOf(parseInt(m.style.getPropertyValue('--moon-phase'))||0),n=(c+1)%ps.length;m.style.setProperty('--moon-phase',ps[n]+'%');const mi=document.getElementById('moonIcon');if(mi)mi.textContent=MOON_PHASES[n].icon;debugLog('Moon cycled')}}
